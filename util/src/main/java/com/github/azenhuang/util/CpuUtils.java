...
public class CpuUtils {
    ...
    public static String printCpuInfo() {
        ...
        try {
            sb =FileUtils.readFile(CPU_INFO_PATH,CharsetUtils.UTF_8.name());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static long getCurrentFreqency() {
        try {
            return Long.parseLong(FileUtils.readFile(CPU_FREQ_CUR_PATH,CharsetUtils.UTF_8.name()).toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
